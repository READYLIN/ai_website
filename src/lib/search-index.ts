import { getStoredArticles } from './storage';
...
async function loadDocs(): Promise<SearchDoc[]> {
  ...
  try {
    // Use Redis-stored articles (fast, no live RSS fetch)
    const articles = await getStoredArticles().catch(() => [] as any[]);
    for (const a of articles) {
      docs.push({
        id: a.id, title: a.title || '', titleZh: a.titleZh || '',
        description: a.description || '', descriptionZh: a.descriptionZh || '',
        source: a.source || '', categories: (a.categories || []).join(' '),
        type: 'article', url: a.url || '', publishedAt: a.publishedAt || ''
      });
    }
  } catch {}
  ...
  try {
    ...
    for (const m of media) {
      docs.push({
        ...
        description: m.description || '', descriptionZh: m.description || '',
        source: m.source || '', categories: (m.categories || []).join(' '),
        type: 'media', url: m.url || '', publishedAt: m.publishedAt || '', company: m.company || ''
      });
    }
  } catch {}
  ...
  try {
    ...
    for (const p of pe) {
      docs.push({
        ...
        description: p.description || '', descriptionZh: p.description || '',
        source: p.source || '', categories: (p.categories || []).join(' '),
        type: 'pe', url: p.url || '', publishedAt: p.publishedAt || '', company: p.company || ''
      });
    }
  } catch {}
