// 识澜·账户系统 — users 表迁移执行器
// 读取 .env.local 中的 MYSQL_* 配置，执行 scripts/migrate-auth.sql。
...
const sqlPath = join(ROOT, 'scripts', 'migrate-auth.sql');
...
async function main() {
  ...
  try {
    ...
    console.log('[auth] migration applied (idempotent). Table ensured: users');
    const [rows] = await conn.query(
      "SELECT table_name FROM information_schema.tables WHERE table_schema = ? AND table_name = 'users'",
      [config.database],
    );
    if (Array.isArray(rows) && rows.length > 0) {
      console.log('[auth] OK: users exists.');
    } else {
      console.error('[auth] WARNING: users not found after migration.');
    }
  } finally {
    await conn.end();
  }
